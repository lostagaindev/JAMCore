...
with no depth in y ("into the screen" if viewed from the front).
We then have functions to return getHeight and getWidth carefully explaining what they are.

We also need to transfere the intersection function, which is strictly 2D. Again, carefully explain its for 2d use only (that is, takes no account of "y"). Its still required for z-index decuding on 2d games, however.
...
public class CollisionBox {
	...
	public static Logger Log = Logger.getLogger("JAMCore.CollisionBox");


	public Simple2DPoint topleft     = new Simple2DPoint(0,0);
	public Simple2DPoint bottomright = new Simple2DPoint(0,0);

	public CollisionBox(Simple2DPoint topleft, Simple2DPoint bottomright) {
		super();
